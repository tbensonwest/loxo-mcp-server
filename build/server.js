    ...
    try {
        switch (name) {
            ...
            case "loxo_create_candidate": {
                ...
                if (email)
                    formData.append('person[email_addresses][][value]', email);
                if (phone)
                    formData.append('person[phone_numbers][][value]', phone);
                if (current_title)
                    formData.append('person[title]', current_title);
                if (current_company)
                    formData.append('person[company]', current_company);
                if (location)
                    formData.append('person[location]', location);
                if (person_type_id)
                    formData.append('person[person_type_ids][]', person_type_id.toString());
                if (tags)
                    formData.append('person[tag_list]', tags);
                const response = await makeRequest(`/${env.LOXO_AGENCY_SLUG}/people`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                    body: formData.toString(),
                });
            ...
            case "loxo_update_candidate": {
                ...
                const formData = new URLSearchParams();
                if (updateName)
                    formData.append('person[name]', updateName);
                if (email)
                    formData.append('person[email_addresses][][value]', email);
                if (phone)
                    formData.append('person[phone_numbers][][value]', phone);
            ...
            case "loxo_get_candidate_activities": {
                const { person_id, per_page, scroll_id, response_format = 'json' } = args;
                ...
                const apiResponse = await makeRequest(`/${env.LOXO_AGENCY_SLUG}/person_events?${params.toString()}`);
            ...
            case "loxo_get_candidate_brief": {
                ...
                const [profileResult, emailsResult, phonesResult, activitiesResult] = await Promise.allSettled([
                    ...
                    makeRequest(`/${env.LOXO_AGENCY_SLUG}/people/${id}/phones`),
                    makeRequest(`/${env.LOXO_AGENCY_SLUG}/person_events?person_id=${id}&per_page=5`),
                ]);
                ...
                const formatted = formatResponse(brief, response_format);
                const { text } = truncateResponse(formatted);
                return { content: [{ type: "text", text }] };
            }
            case "loxo_get_job_pipeline": {
                ...
                if (scroll_id)
                    params.append('scroll_id', scroll_id);
                const apiResponse = await makeRequest(`/${env.LOXO_AGENCY_SLUG}/job_contacts?${params.toString()}`);
            ...
            case "loxo_apply_to_job": {
                ...
                const response = await makeRequest(`/${env.LOXO_AGENCY_SLUG}/job_contacts`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                    body: formData.toString(),
                });
